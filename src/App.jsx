...
// Auto-detect chain from coupon code prefix
const DETECT = [
  { r: /^RL[-\s]/i,        c: "רמי לוי" },
...
function Logo({ chain }) {
  ...
  switch (chain) {
    case "רמי לוי":
      return <div style={{ ...box, background: "#e8001a", flexDirection: "column" }}>
        <span style={{ color: "#fff", fontSize: 10, fontWeight: 900, lineHeight: 1.1, textAlign: "center" }}>רמי<br />לוי</span>
      </div>;
    case "סופר קרפור":
      return <div style={{ ...box, background: "#003da5" }}>
        <svg width="36" height="26" viewBox="0 0 90 60">
          <polygon points="45,30 20,5 20,55" fill="#ed1c24" />
          <polygon points="45,30 70,5 70,55" fill="#ed1c24" />
          <rect x="0" y="0" width="20" height="60" fill="#fff" />
          <rect x="70" y="0" width="20" height="60" fill="#fff" />
        </svg>
      </div>;
    case "שופרסל":
      return <div style={{ ...box, background: "#00873c" }}>
        <span style={{ color: "#fff", fontSize: 11, fontWeight: 900, textAlign: "center", lineHeight: 1.1 }}>שופר<br />סל</span>
      </div>;
    case "ויקטורי":
      return <div style={{ ...box, background: "#f97316" }}>
        <span style={{ color: "#fff", fontSize: 9, fontWeight: 900, textAlign: "center", lineHeight: 1.1 }}>ויק<br />טורי</span>
      </div>;
    case "מגה ספורט":
      return <div style={{ ...box, background: "#fff", flexDirection: "column", padding: 3 }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 1, lineHeight: 1 }}>
          <span style={{ color: "#e8001a", fontSize: 14, fontWeight: 900, fontStyle: "italic" }}>מ</span>
          <span style={{ color: "#2d7a2d", fontSize: 10, fontWeight: 900, fontStyle: "italic" }}>גה</span>
        </div>
        <span style={{ color: "#2d7a2d", fontSize: 8, fontWeight: 900, fontStyle: "italic", letterSpacing: -0.3 }}>ספורט</span>
      </div>;
    case "מחסני להב":
      return <div style={{ ...box, background: "#7c1c1c" }}>
        <span style={{ color: "#f5c842", fontSize: 9, fontWeight: 900, textAlign: "center", lineHeight: 1.2 }}>מחסני<br />להב</span>
      </div>;
    case "BOOM":
      return <div style={{ ...box, background: "#fbbf24" }}>
        <span style={{ color: "#1a1a1a", fontSize: 11, fontWeight: 900 }}>💥BOOM</span>
      </div>;
    case "חבר":
      return <div style={{ ...box, background: "#1e40af" }}>
        <span style={{ color: "#fff", fontSize: 14, fontWeight: 900 }}>חבר</span>
      </div>;
    case "מקדונלד'ס":
      return <div style={{ ...box, background: "#da0004" }}>
        <svg width="28" height="24" viewBox="0 0 28 24">
          <path d="M4,22 Q4,2 8,2 Q11,2 14,12 Q17,2 20,2 Q24,2 24,22" stroke="#ffbc0d" strokeWidth="4.5" fill="none" strokeLinecap="round" />
        </svg>
      </div>;
    case "פיצה שמש":
      return <div style={{ ...box, background: "#fef3c7", flexDirection: "column" }}>
        <span style={{ fontSize: 18, lineHeight: 1 }}>☀️</span>
        <span style={{ color: "#92400e", fontSize: 7, fontWeight: 900 }}>שמש</span>
      </div>;
    case "פיצה סטורי":
      return <div style={{ ...box, background: "#dc2626", flexDirection: "column", gap: 1 }}>
        <span style={{ fontSize: 16, lineHeight: 1 }}>🍕</span>
        <span style={{ color: "#fff", fontSize: 7, fontWeight: 900 }}>סטורי</span>
      </div>;
    case "פיצה האט":
      return <div style={{ ...box, background: "#cc0000", flexDirection: "column" }}>
        <span style={{ color: "#fff", fontSize: 7, fontWeight: 900, lineHeight: 1.2, textAlign: "center" }}>PIZZA<br />HUT</span>
      </div>;
    case "דומינו'ס":
      return <div style={{ ...box, background: "#006491" }}>
        <svg width="30" height="30" viewBox="0 0 30 30">
          <rect x="1" y="1" width="12" height="28" rx="2" fill="#e31837" />
          <rect x="15" y="1" width="14" height="13" rx="2" fill="#e31837" />
          <rect x="15" y="16" width="14" height="13" rx="2" fill="#fff" opacity="0.25" />
    ...
    case "יינות ביתן":
      return <div style={{ ...box, background: "#7e22ce", flexDirection: "column", gap: 1 }}>
        <span style={{ fontSize: 16, lineHeight: 1 }}>🍷</span>
        <span style={{ color: "#e9d5ff", fontSize: 7, fontWeight: 900, lineHeight: 1.1, textAlign: "center" }}>יינות<br />ביתן</span>
      </div>;
    default:
      return <div style={{ ...box, background: "#374151" }}>
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: "linear-gradient(160deg,#1a1a2e,#16213e)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "18px 18px 0 0", width: "100%", maxWidth: 480, maxHeight: "92vh", overflowY: "auto", padding: "14px 16px 26px" }}
      >
...
// ─── Coupon Card ──────────────────────────────────────────────────
function CouponCard({ coupon: c, open, onToggle, onMarkUsed, onEdit, onDelete, onBalance }) {
    ...
    <div style={{ borderRadius: 13, background: "rgba(255,255,255,0.045)", overflow: "hidden", marginBottom: 9, border: `1px solid ${bc}33`, borderRight: `4px solid ${bc}` }}>
      ...
      <div onClick={onToggle} style={{ display: "flex", alignItems: "center", gap: 10, padding: 12, cursor: "pointer", userSelect: "none" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 700, fontSize: 13, display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
            ...
            {hasBal && !c.used && (
              <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 20, fontWeight: 700, background: "rgba(99,179,237,0.14)", color: "#63b3ed", border: "1px solid #63b3ed44" }}>💳</span>
            )}
            {c.image && (
              <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 20, background: "rgba(255,255,255,0.07)", color: "rgba(255,255,255,0.4)" }}>📷</span>
            )}
          </div>
        ...
        <div style={{ textAlign: "center", fontSize: 9, color: "rgba(255,255,255,0.32)", flexShrink: 0, lineHeight: 1.6 }}>
          {c.expiry && (
            <>
              <div>עד</div>
              <div style={{ fontWeight: 700, color: isSoon(c.expiry) && !c.used ? "#ff9800" : undefined }}>{c.expiry}</div>
            </>
          )}
          <div>👤 {c.addedBy}</div>
      ...
      {open && (
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.05)", padding: "10px 12px 12px" }}>
          {c.image && (
            <img src={c.image} alt="שובר" style={{ width: "100%", maxHeight: 140, objectFit: "contain", borderRadius: 8, marginBottom: 10, background: "rgba(0,0,0,0.3)", display: "block" }} />
          )}
          {c.used && c.usedDate && (
            <div style={{ fontSize: 10, color: "#ab47bc", marginBottom: 8 }}>📅 נוצל בתאריך: {c.usedDate}</div>
          )}
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {hasBal && !c.used && (
              <button onClick={onBalance} style={{ flex: 2, padding: 8, borderRadius: 9, background: "rgba(99,179,237,0.12)", border: "1px solid #63b3ed", color: "#63b3ed", fontWeight: 700, fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>
                💳 בדוק יתרה
              </button>
            )}
            {!c.used && (
              <button onClick={onMarkUsed} style={{ flex: 2, padding: 8, borderRadius: 9, border: "none", background: "linear-gradient(135deg,#3182ce,#2563eb)", color: "#fff", fontWeight: 700, fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>
                ✅ סמן נוצל
              </button>
            )}
            <button onClick={onEdit} style={{ padding: "8px 10px", borderRadius: 9, background: "rgba(245,158,11,0.12)", border: "1px solid #f59e0b", color: "#f59e0b", fontWeight: 700, fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>
              ✏️ ערוך
            </button>
            <button onClick={onDelete} style={{ padding: "8px 10px", borderRadius: 9, background: "rgba(239,68,68,0.1)", border: "1px solid #ef4444", color: "#ef4444", fontWeight: 700, fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>
              🗑️ מחק
            </button>
          </div>
        </div>
...
export default function App() {
  ...
  const [balModal, setBalModal]     = useState(null); // { chain, url }
  const [form, setForm]             = useState({ chain: "", code: "", expiry: "", discount: "", addedBy: "אני" });
  const [imgPreview, setImgPreview] = useState(null);
  const [analyzing, setAnalyzing]   = useState(false);
  const [aiMsg, setAiMsg]           = useState(null); // { text, type: "ok"|"warn"|"err" }
          ...
          body: JSON.stringify({
            ...
            messages: [{
              ...
              content: [
                ...
                { type: "text", text: `אתה מנתח שוברים וקופונים ישראלים. נתח את התמונה וחלץ:
1. שם הרשת (חפש לוגו/שם מותג)
2. מספר/קוד השובר (סדרת מספרים או אותיות ומספרים)
3. תאריך תפוגה
4. ערך/הטבה

החזר JSON בלבד ללא markdown:
{"chain":"שם מהרשימה: ${CHAIN_LIST.join(", ")} — אם לא זוהה השתמש באחר","code":"הקוד/מספר","expiry":"YYYY-MM-DD","discount":"ערך/הטבה"}

חשוב: גם אם לא זיהית את הרשת, חלץ את מספר השובר!` }
              ]
            }]
          }),
        });
        ...
        if (noCode && noChain)  setAiMsg({ text: "⚠️ לא הצלחתי לקרוא פרטים — מלא ידנית", type: "warn" });
        else if (noChain)       setAiMsg({ text: "✨ קוד זוהה! לא זיהיתי את הרשת — בחר ידנית", type: "warn" });
        else                    setAiMsg({ text: "✨ AI מילא את הפרטים — בדוק ותקן אם צריך", type: "ok" });
      } catch {
        setAiMsg({ text: "❌ שגיאה בניתוח — בדוק חיבור ומלא ידנית", type: "err" });
      }
      setAnalyzing(false);
    };
  ...
  // ── Open add/edit ─────────────────────────────────────────────────
  const openAdd = () => {
    setEditId(null); setImgPreview(null); setAiMsg(null);
    setForm({ chain: "", code: "", expiry: "", discount: "", addedBy: "אני" });
    setShowForm(true);
  };

  const openEdit = (c) => {
    setEditId(c.id); setImgPreview(c.image || null); setAiMsg(null);
    ...
    try {
      if (editId) {
        await updateDoc(doc(db, "coupons", editId), {
          chain: form.chain, code: form.code,
  ...
  // ─── Render ───────────────────────────────────────────────────────
  const aiBgMap = { ok: "rgba(99,179,237,0.1)", warn: "rgba(245,158,11,0.1)", err: "rgba(239,68,68,0.1)" };
  const aiBrMap = { ok: "rgba(99,179,237,0.3)", warn: "rgba(245,158,11,0.3)", err: "rgba(239,68,68,0.3)" };
  const aiClMap = { ok: "#63b3ed", warn: "#fcd34d", err: "#fc8181" };
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(145deg,#0d1b2a,#1a2838)", fontFamily: "'Heebo','Segoe UI',Arial,sans-serif", direction: "rtl", color: "#e8f0fe" }}>
      ...
      <div style={{ background: "rgba(255,255,255,0.04)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,0.09)", padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 10 }}>
        <div>
          ...
          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", marginTop: 1 }}>
            {loading ? "🔄 מתחבר לענן..." : `✅ מסונכרן • ${active.length} שוברים פעילים`}
          </div>
        </div>
        <button onClick={openAdd} style={{ background: "linear-gradient(135deg,#3182ce,#2563eb)", border: "none", borderRadius: 10, padding: "9px 16px", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>
          + הוסף שובר
        </button>
      </div>
      ...
      <div style={{ display: "flex", gap: 7, padding: "10px 14px 0" }}>
        {[{ k: "active", l: "📋 פעילים", n: active.length }, { k: "archive", l: "📦 ארכיון", n: archived.length }].map((t) => (
          <button key={t.k} onClick={() => { setTab(t.k); setExpandedId(null); }} style={{
      ...
      <div style={{ padding: "10px 14px 80px" }}>
        {loading ? (
          <div style={{ textAlign: "center", padding: 60, color: "rgba(255,255,255,0.3)", fontSize: 14 }}>🔄 טוען שוברים...</div>
        ) : list.length === 0 ? (
          <div style={{ textAlign: "center", padding: 60, color: "rgba(255,255,255,0.22)", fontSize: 13, lineHeight: 3 }}>
            {tab === "active" ? "🎟️" : "📦"}<br />
            {tab === "active" ? "אין שוברים פעילים\nלחץ + הוסף שובר כדי להתחיל" : "הארכיון ריק"}
          </div>
        ) : list.map((c) => (
          <CouponCard
            key={c.id}
            ...
            onBalance={() => setBalModal({ chain: c.chain, url: brand(c.chain).balUrl })}
          />
        ))}
      </div>
      ...
      {showForm && (
        <Modal onClose={closeForm}>
          <div style={{ fontSize: 16, fontWeight: 800, marginBottom: 12 }}>
            {editId
              ? <>✏️ עריכת שובר <span style={{ fontSize: 11, fontWeight: 500, color: "#f59e0b", background: "rgba(245,158,11,0.1)", border: "1px solid #f59e0b44", padding: "2px 8px", borderRadius: 20 }}>עריכה</span></>
              : "➕ שובר חדש"}
          </div>

          {/* Upload — only in add mode */}
          {!editId && (
            <>
              <div
                onClick={() => fileRef.current.click()}
                style={{ border: "2px dashed rgba(99,179,237,0.4)", borderRadius: 12, padding: 16, textAlign: "center", cursor: "pointer", marginBottom: 10, position: "relative", overflow: "hidden", minHeight: 80, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4 }}
              >
                {imgPreview
                  ...
                  : <>
                      ...
                      <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)" }}>AI ימלא את הפרטים אוטומטית ✨</div>
                    </>}
                {analyzing && (
              ...
              {aiMsg && (
                <div style={{ background: aiBgMap[aiMsg.type], border: `1px solid ${aiBrMap[aiMsg.type]}`, borderRadius: 8, padding: "6px 10px", fontSize: 11, color: aiClMap[aiMsg.type], marginBottom: 10 }}>
                  {aiMsg.text}
                </div>
              )}
            </>
          ...
          {editId && imgPreview && (
            <img src={imgPreview} alt="" style={{ width: "100%", maxHeight: 120, objectFit: "contain", borderRadius: 8, marginBottom: 12, background: "rgba(0,0,0,0.3)" }} />
          )}
            ...
            <div key={f.key} style={{ marginBottom: 10 }}>
              ...
              {f.type === "select" ? (
                ...
                <input
                  type={f.type || "text"}
                  value={form[f.key]}
                  onChange={(e) => f.key === "code" ? onCodeChange(e.target.value) : setField(f.key, e.target.value)}
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", gap: 7 }}>
              {["אני", "אישתי"].map((w) => (
                <button key={w} onClick={() => setField("addedBy", w)} style={{
                  flex: 1, padding: 8, borderRadius: 9, fontWeight: 600, fontSize: 12,
                  cursor: "pointer", fontFamily: "inherit",
                  ...(form.addedBy === w ? whoOn : whoOff),
                }}>{w}</button>
              ))}
            </div>
          </div>
          ...
          <button onClick={save} style={{
            width: "100%", padding: 12, borderRadius: 11, border: "none", color: "#fff",
            fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "inherit",
            background: editId ? "linear-gradient(135deg,#d97706,#b45309)" : "linear-gradient(135deg,#3182ce,#2563eb)",
          }}>
            {editId ? "💾 עדכן שובר" : "💾 שמור שובר"}
          </button>
          <button onClick={closeForm} style={{ width: "100%", padding: 9, borderRadius: 11, border: "1px solid rgba(255,255,255,0.14)", background: "transparent", color: "rgba(255,255,255,0.5)", fontWeight: 600, fontSize: 13, cursor: "pointer", marginTop: 7, fontFamily: "inherit" }}>
            ביטול
          </button>
        </Modal>
      )}
      ...
      {balModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", flexDirection: "column" }}>
          <div style={{ background: "#1a1a2e", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.09)", flexShrink: 0 }}>
            <div>
              <div style={{ fontWeight: 700, fontSize: 14, color: "#fff" }}>💳 בדיקת יתרה</div>
              <div style={{ fontSize: 10, color: "rgba(255,255,255,0.42)", marginTop: 1 }}>{balModal.chain}</div>
            </div>
